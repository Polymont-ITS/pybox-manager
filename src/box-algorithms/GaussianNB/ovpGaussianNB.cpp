...
OpenViBE::boolean CBoxAlgorithmGaussianNB::logSysStdout(void)
{
	//New reference
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::logSysStderr(void)
{
	//New reference
...
uint64 CBoxAlgorithmGaussianNB::getClockFrequency(void)
{
...
void CBoxAlgorithmGaussianNB::buildPythonSettings(void)
{  
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::initializePythonSafely()
{
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::initialize(void)
{
	m_pBox = nullptr;
	m_pBoxInput = nullptr;
	...
	for(uint32 input=0; input<l_rStaticBoxContext.getInputCount(); input++)
	{
		...
		{
			m_vDecoders.push_back( new TStreamedMatrixDecoder <CBoxAlgorithmGaussianNB> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Signal)
		{
			m_vDecoders.push_back( new TSignalDecoder <CBoxAlgorithmGaussianNB> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_FeatureVector)
		{
			m_vDecoders.push_back( new TFeatureVectorDecoder <CBoxAlgorithmGaussianNB> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Spectrum)
		{
			m_vDecoders.push_back( new TSpectrumDecoder <CBoxAlgorithmGaussianNB> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ChannelLocalisation)
		{
			m_vDecoders.push_back( new TChannelLocalisationDecoder <CBoxAlgorithmGaussianNB> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Stimulations)
		{
			m_vDecoders.push_back( new TStimulationDecoder <CBoxAlgorithmGaussianNB> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ExperimentInformation)
		{
			m_vDecoders.push_back( new TExperimentInformationDecoder <CBoxAlgorithmGaussianNB> (*this, input) );
		}
		else
		{
	...
	for(uint32 output=0; output<l_rStaticBoxContext.getOutputCount(); output++)
	{
		...
		if (l_oTypeIdentifier == OV_TypeId_StreamedMatrix)
		{
			m_vEncoders.push_back( new TStreamedMatrixEncoder <CBoxAlgorithmGaussianNB> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Signal)
		{
			m_vEncoders.push_back( new TSignalEncoder <CBoxAlgorithmGaussianNB> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_FeatureVector)
		{
			m_vEncoders.push_back( new TFeatureVectorEncoder <CBoxAlgorithmGaussianNB> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Spectrum)
		{
			m_vEncoders.push_back( new TSpectrumEncoder <CBoxAlgorithmGaussianNB> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ChannelLocalisation)
		{
			m_vEncoders.push_back( new TChannelLocalisationEncoder <CBoxAlgorithmGaussianNB> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Stimulations)
		{
			m_vEncoders.push_back( new TStimulationEncoder <CBoxAlgorithmGaussianNB> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ExperimentInformation)
		{
			m_vEncoders.push_back( new TExperimentInformationEncoder <CBoxAlgorithmGaussianNB> (*this, output) );
		}
		else
		{
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::uninitialize(void)
{
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::processClock(CMessageClock& rMessageClock)
{
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::processInput(uint32 ui32InputIndex)
{
	getBoxAlgorithmContext()->markAlgorithmAsReadyToProcess();
	return true;
}

OpenViBE::boolean CBoxAlgorithmGaussianNB::transferStreamedMatrixInputChunksToPython(const uint32 input_index)
{
	...
	for(uint32 chunk_index=0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		if (m_vDecoders[input_index]->isHeaderReceived())
		{
			...
			IMatrix* l_pMatrix = ( (TStreamedMatrixDecoder <CBoxAlgorithmGaussianNB> *) m_vDecoders[input_index] )->getOutputMatrix();
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::transferStreamedMatrixOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IMatrix* l_pMatrix = ( (TStreamedMatrixEncoder <CBoxAlgorithmGaussianNB> *) m_vEncoders[output_index] )->getInputMatrix();
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::transferSignalInputChunksToPython(const uint32 input_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
	...
	for(uint32 chunk_index=0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		{
			uint32 l_ui32DimensionCount, l_ui32DimensionSize;
			IMatrix* l_pMatrix = ( (TSignalDecoder <CBoxAlgorithmGaussianNB> *) m_vDecoders[input_index] )->getOutputMatrix();
			l_ui32DimensionCount = l_pMatrix->getDimensionCount();
			...
			if (PyTuple_SetItem(l_pArg, 4, PyInt_FromLong((long) ( (TSignalDecoder <CBoxAlgorithmGaussianNB> *) m_vDecoders[input_index] )->getOutputSamplingRate())) != 0)
			{
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::transferSignalOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IMatrix* l_pMatrix = ( (TSignalEncoder <CBoxAlgorithmGaussianNB> *) m_vEncoders[output_index] )->getInputMatrix();
	...
	for (uint32 chunk_index=0; chunk_index < l_ui32OutputLen; chunk_index++)
	{
		...
		if (PyObject_IsInstance(l_pOVChunk, CBoxAlgorithmPython::m_pOVSignalHeader) == 1)
		{
			...
			OpenViBE::Kernel::TParameterHandler < OpenViBE::uint64 >& l_pSamplingRate = ( (TSignalEncoder <CBoxAlgorithmGaussianNB> *) m_vEncoders[output_index] )->getInputSamplingRate();
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::transferStimulationInputChunksToPython(const uint32 input_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
	//Borrowed reference
	PyObject *l_pBuffer = PyList_GetItem(m_pBoxInput, (Py_ssize_t) input_index);
	...
	for(uint32 chunk_index = 0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		if (m_vDecoders[input_index]->isBufferReceived())
		{
			IStimulationSet* l_pStimulationSet = ( (TStimulationDecoder <CBoxAlgorithmGaussianNB> *) m_vDecoders[input_index] )->getOutputStimulationSet();
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::transferStimulationOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IStimulationSet* l_pStimulationSet = ( (TStimulationEncoder <CBoxAlgorithmGaussianNB> *) m_vEncoders[output_index] )->getInputStimulationSet();
...
OpenViBE::boolean CBoxAlgorithmGaussianNB::process(void)
{	
    const IBox& l_rStaticBoxContext = this->getStaticBoxContext();
	OpenViBE::CIdentifier l_oTypeIdentifier;
