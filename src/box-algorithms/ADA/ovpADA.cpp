...
OpenViBE::boolean CBoxAlgorithmADA::logSysStdout(void)
{
	//New reference
...
OpenViBE::boolean CBoxAlgorithmADA::logSysStderr(void)
{
	//New reference
...
uint64 CBoxAlgorithmADA::getClockFrequency(void)
{
...
void CBoxAlgorithmADA::buildPythonSettings(void)
{  
...
OpenViBE::boolean CBoxAlgorithmADA::initializePythonSafely()
{
...
OpenViBE::boolean CBoxAlgorithmADA::initialize(void)
{
	m_pBox = nullptr;
	m_pBoxInput = nullptr;
	...
	for(uint32 input=0; input<l_rStaticBoxContext.getInputCount(); input++)
	{
		...
		{
			m_vDecoders.push_back( new TStreamedMatrixDecoder <CBoxAlgorithmADA> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Signal)
		{
			m_vDecoders.push_back( new TSignalDecoder <CBoxAlgorithmADA> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_FeatureVector)
		{
			m_vDecoders.push_back( new TFeatureVectorDecoder <CBoxAlgorithmADA> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Spectrum)
		{
			m_vDecoders.push_back( new TSpectrumDecoder <CBoxAlgorithmADA> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ChannelLocalisation)
		{
			m_vDecoders.push_back( new TChannelLocalisationDecoder <CBoxAlgorithmADA> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Stimulations)
		{
			m_vDecoders.push_back( new TStimulationDecoder <CBoxAlgorithmADA> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ExperimentInformation)
		{
			m_vDecoders.push_back( new TExperimentInformationDecoder <CBoxAlgorithmADA> (*this, input) );
		}
		else
		{
	...
	for(uint32 output=0; output<l_rStaticBoxContext.getOutputCount(); output++)
	{
		...
		if (l_oTypeIdentifier == OV_TypeId_StreamedMatrix)
		{
			m_vEncoders.push_back( new TStreamedMatrixEncoder <CBoxAlgorithmADA> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Signal)
		{
			m_vEncoders.push_back( new TSignalEncoder <CBoxAlgorithmADA> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_FeatureVector)
		{
			m_vEncoders.push_back( new TFeatureVectorEncoder <CBoxAlgorithmADA> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Spectrum)
		{
			m_vEncoders.push_back( new TSpectrumEncoder <CBoxAlgorithmADA> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ChannelLocalisation)
		{
			m_vEncoders.push_back( new TChannelLocalisationEncoder <CBoxAlgorithmADA> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Stimulations)
		{
			m_vEncoders.push_back( new TStimulationEncoder <CBoxAlgorithmADA> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ExperimentInformation)
		{
			m_vEncoders.push_back( new TExperimentInformationEncoder <CBoxAlgorithmADA> (*this, output) );
		}
		else
		{
...
OpenViBE::boolean CBoxAlgorithmADA::uninitialize(void)
{
...
OpenViBE::boolean CBoxAlgorithmADA::processClock(CMessageClock& rMessageClock)
{
...
OpenViBE::boolean CBoxAlgorithmADA::processInput(uint32 ui32InputIndex)
{
	getBoxAlgorithmContext()->markAlgorithmAsReadyToProcess();
	return true;
}

OpenViBE::boolean CBoxAlgorithmADA::transferStreamedMatrixInputChunksToPython(const uint32 input_index)
{
	...
	for(uint32 chunk_index=0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		if (m_vDecoders[input_index]->isHeaderReceived())
		{
			...
			IMatrix* l_pMatrix = ( (TStreamedMatrixDecoder <CBoxAlgorithmADA> *) m_vDecoders[input_index] )->getOutputMatrix();
...
OpenViBE::boolean CBoxAlgorithmADA::transferStreamedMatrixOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IMatrix* l_pMatrix = ( (TStreamedMatrixEncoder <CBoxAlgorithmADA> *) m_vEncoders[output_index] )->getInputMatrix();
...
OpenViBE::boolean CBoxAlgorithmADA::transferSignalInputChunksToPython(const uint32 input_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
	...
	for(uint32 chunk_index=0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		{
			uint32 l_ui32DimensionCount, l_ui32DimensionSize;
			IMatrix* l_pMatrix = ( (TSignalDecoder <CBoxAlgorithmADA> *) m_vDecoders[input_index] )->getOutputMatrix();
			l_ui32DimensionCount = l_pMatrix->getDimensionCount();
			...
			if (PyTuple_SetItem(l_pArg, 4, PyInt_FromLong((long) ( (TSignalDecoder <CBoxAlgorithmADA> *) m_vDecoders[input_index] )->getOutputSamplingRate())) != 0)
			{
...
OpenViBE::boolean CBoxAlgorithmADA::transferSignalOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IMatrix* l_pMatrix = ( (TSignalEncoder <CBoxAlgorithmADA> *) m_vEncoders[output_index] )->getInputMatrix();
	...
	for (uint32 chunk_index=0; chunk_index < l_ui32OutputLen; chunk_index++)
	{
		...
		if (PyObject_IsInstance(l_pOVChunk, CBoxAlgorithmPython::m_pOVSignalHeader) == 1)
		{
			...
			OpenViBE::Kernel::TParameterHandler < OpenViBE::uint64 >& l_pSamplingRate = ( (TSignalEncoder <CBoxAlgorithmADA> *) m_vEncoders[output_index] )->getInputSamplingRate();
...
OpenViBE::boolean CBoxAlgorithmADA::transferStimulationInputChunksToPython(const uint32 input_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
	//Borrowed reference
	PyObject *l_pBuffer = PyList_GetItem(m_pBoxInput, (Py_ssize_t) input_index);
	...
	for(uint32 chunk_index = 0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		if (m_vDecoders[input_index]->isBufferReceived())
		{
			IStimulationSet* l_pStimulationSet = ( (TStimulationDecoder <CBoxAlgorithmADA> *) m_vDecoders[input_index] )->getOutputStimulationSet();
...
OpenViBE::boolean CBoxAlgorithmADA::transferStimulationOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IStimulationSet* l_pStimulationSet = ( (TStimulationEncoder <CBoxAlgorithmADA> *) m_vEncoders[output_index] )->getInputStimulationSet();
...
OpenViBE::boolean CBoxAlgorithmADA::process(void)
{	
    const IBox& l_rStaticBoxContext = this->getStaticBoxContext();
	OpenViBE::CIdentifier l_oTypeIdentifier;
