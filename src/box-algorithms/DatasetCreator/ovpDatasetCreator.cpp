...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::logSysStdout(void)
{
	//New reference
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::logSysStderr(void)
{
	//New reference
...
uint64 CBoxAlgorithmDatasetCreator::getClockFrequency(void)
{
...
void CBoxAlgorithmDatasetCreator::buildPythonSettings(void)
{  
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::initializePythonSafely()
{
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::initialize(void)
{
	m_pBox = nullptr;
	m_pBoxInput = nullptr;
	...
	m_sScriptFilename = "../../extras/contrib/applications/developer-tools/pybox-manager/ScriptBox/DatasetCreator.py";
	...
	for(uint32 input=0; input<l_rStaticBoxContext.getInputCount(); input++)
	{
		...
		{
			m_vDecoders.push_back( new TStreamedMatrixDecoder <CBoxAlgorithmDatasetCreator> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Signal)
		{
			m_vDecoders.push_back( new TSignalDecoder <CBoxAlgorithmDatasetCreator> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_FeatureVector)
		{
			m_vDecoders.push_back( new TFeatureVectorDecoder <CBoxAlgorithmDatasetCreator> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Spectrum)
		{
			m_vDecoders.push_back( new TSpectrumDecoder <CBoxAlgorithmDatasetCreator> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ChannelLocalisation)
		{
			m_vDecoders.push_back( new TChannelLocalisationDecoder <CBoxAlgorithmDatasetCreator> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Stimulations)
		{
			m_vDecoders.push_back( new TStimulationDecoder <CBoxAlgorithmDatasetCreator> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ExperimentInformation)
		{
			m_vDecoders.push_back( new TExperimentInformationDecoder <CBoxAlgorithmDatasetCreator> (*this, input) );
		}
		else
		{
	...
	for(uint32 output=0; output<l_rStaticBoxContext.getOutputCount(); output++)
	{
		...
		if (l_oTypeIdentifier == OV_TypeId_StreamedMatrix)
		{
			m_vEncoders.push_back( new TStreamedMatrixEncoder <CBoxAlgorithmDatasetCreator> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Signal)
		{
			m_vEncoders.push_back( new TSignalEncoder <CBoxAlgorithmDatasetCreator> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_FeatureVector)
		{
			m_vEncoders.push_back( new TFeatureVectorEncoder <CBoxAlgorithmDatasetCreator> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Spectrum)
		{
			m_vEncoders.push_back( new TSpectrumEncoder <CBoxAlgorithmDatasetCreator> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ChannelLocalisation)
		{
			m_vEncoders.push_back( new TChannelLocalisationEncoder <CBoxAlgorithmDatasetCreator> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Stimulations)
		{
			m_vEncoders.push_back( new TStimulationEncoder <CBoxAlgorithmDatasetCreator> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ExperimentInformation)
		{
			m_vEncoders.push_back( new TExperimentInformationEncoder <CBoxAlgorithmDatasetCreator> (*this, output) );
		}
		else
		{
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::uninitialize(void)
{
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::processClock(CMessageClock& rMessageClock)
{
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::processInput(uint32 ui32InputIndex)
{
	getBoxAlgorithmContext()->markAlgorithmAsReadyToProcess();
	return true;
}

OpenViBE::boolean CBoxAlgorithmDatasetCreator::transferStreamedMatrixInputChunksToPython(const uint32 input_index)
{
	...
	for(uint32 chunk_index=0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		if (m_vDecoders[input_index]->isHeaderReceived())
		{
			...
			IMatrix* l_pMatrix = ( (TStreamedMatrixDecoder <CBoxAlgorithmDatasetCreator> *) m_vDecoders[input_index] )->getOutputMatrix();
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::transferStreamedMatrixOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IMatrix* l_pMatrix = ( (TStreamedMatrixEncoder <CBoxAlgorithmDatasetCreator> *) m_vEncoders[output_index] )->getInputMatrix();
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::transferSignalInputChunksToPython(const uint32 input_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
	...
	for(uint32 chunk_index=0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		{
			uint32 l_ui32DimensionCount, l_ui32DimensionSize;
			IMatrix* l_pMatrix = ( (TSignalDecoder <CBoxAlgorithmDatasetCreator> *) m_vDecoders[input_index] )->getOutputMatrix();
			l_ui32DimensionCount = l_pMatrix->getDimensionCount();
			...
			if (PyTuple_SetItem(l_pArg, 4, PyInt_FromLong((long) ( (TSignalDecoder <CBoxAlgorithmDatasetCreator> *) m_vDecoders[input_index] )->getOutputSamplingRate())) != 0)
			{
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::transferSignalOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IMatrix* l_pMatrix = ( (TSignalEncoder <CBoxAlgorithmDatasetCreator> *) m_vEncoders[output_index] )->getInputMatrix();
	...
	for (uint32 chunk_index=0; chunk_index < l_ui32OutputLen; chunk_index++)
	{
		...
		if (PyObject_IsInstance(l_pOVChunk, CBoxAlgorithmPython::m_pOVSignalHeader) == 1)
		{
			...
			OpenViBE::Kernel::TParameterHandler < OpenViBE::uint64 >& l_pSamplingRate = ( (TSignalEncoder <CBoxAlgorithmDatasetCreator> *) m_vEncoders[output_index] )->getInputSamplingRate();
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::transferStimulationInputChunksToPython(const uint32 input_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
	//Borrowed reference
	PyObject *l_pBuffer = PyList_GetItem(m_pBoxInput, (Py_ssize_t) input_index);
	...
	for(uint32 chunk_index = 0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		if (m_vDecoders[input_index]->isBufferReceived())
		{
			IStimulationSet* l_pStimulationSet = ( (TStimulationDecoder <CBoxAlgorithmDatasetCreator> *) m_vDecoders[input_index] )->getOutputStimulationSet();
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::transferStimulationOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IStimulationSet* l_pStimulationSet = ( (TStimulationEncoder <CBoxAlgorithmDatasetCreator> *) m_vEncoders[output_index] )->getInputStimulationSet();
...
OpenViBE::boolean CBoxAlgorithmDatasetCreator::process(void)
{	
    const IBox& l_rStaticBoxContext = this->getStaticBoxContext();
	OpenViBE::CIdentifier l_oTypeIdentifier;
