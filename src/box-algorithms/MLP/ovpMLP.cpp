...
OpenViBE::boolean CBoxAlgorithmMLP::logSysStdout(void)
{
	//New reference
...
OpenViBE::boolean CBoxAlgorithmMLP::logSysStderr(void)
{
	//New reference
...
uint64 CBoxAlgorithmMLP::getClockFrequency(void)
{
...
void CBoxAlgorithmMLP::buildPythonSettings(void)
{  
...
OpenViBE::boolean CBoxAlgorithmMLP::initializePythonSafely()
{
...
OpenViBE::boolean CBoxAlgorithmMLP::initialize(void)
{
	m_pBox = nullptr;
	m_pBoxInput = nullptr;
	...
	for(uint32 input=0; input<l_rStaticBoxContext.getInputCount(); input++)
	{
		...
		{
			m_vDecoders.push_back( new TStreamedMatrixDecoder <CBoxAlgorithmMLP> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Signal)
		{
			m_vDecoders.push_back( new TSignalDecoder <CBoxAlgorithmMLP> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_FeatureVector)
		{
			m_vDecoders.push_back( new TFeatureVectorDecoder <CBoxAlgorithmMLP> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Spectrum)
		{
			m_vDecoders.push_back( new TSpectrumDecoder <CBoxAlgorithmMLP> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ChannelLocalisation)
		{
			m_vDecoders.push_back( new TChannelLocalisationDecoder <CBoxAlgorithmMLP> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Stimulations)
		{
			m_vDecoders.push_back( new TStimulationDecoder <CBoxAlgorithmMLP> (*this, input) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ExperimentInformation)
		{
			m_vDecoders.push_back( new TExperimentInformationDecoder <CBoxAlgorithmMLP> (*this, input) );
		}
		else
		{
	...
	for(uint32 output=0; output<l_rStaticBoxContext.getOutputCount(); output++)
	{
		...
		if (l_oTypeIdentifier == OV_TypeId_StreamedMatrix)
		{
			m_vEncoders.push_back( new TStreamedMatrixEncoder <CBoxAlgorithmMLP> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Signal)
		{
			m_vEncoders.push_back( new TSignalEncoder <CBoxAlgorithmMLP> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_FeatureVector)
		{
			m_vEncoders.push_back( new TFeatureVectorEncoder <CBoxAlgorithmMLP> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Spectrum)
		{
			m_vEncoders.push_back( new TSpectrumEncoder <CBoxAlgorithmMLP> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ChannelLocalisation)
		{
			m_vEncoders.push_back( new TChannelLocalisationEncoder <CBoxAlgorithmMLP> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_Stimulations)
		{
			m_vEncoders.push_back( new TStimulationEncoder <CBoxAlgorithmMLP> (*this, output) );
		}
		else if (l_oTypeIdentifier == OV_TypeId_ExperimentInformation)
		{
			m_vEncoders.push_back( new TExperimentInformationEncoder <CBoxAlgorithmMLP> (*this, output) );
		}
		else
		{
...
OpenViBE::boolean CBoxAlgorithmMLP::uninitialize(void)
{
...
OpenViBE::boolean CBoxAlgorithmMLP::processClock(CMessageClock& rMessageClock)
{
...
OpenViBE::boolean CBoxAlgorithmMLP::processInput(uint32 ui32InputIndex)
{
	getBoxAlgorithmContext()->markAlgorithmAsReadyToProcess();
	return true;
}

OpenViBE::boolean CBoxAlgorithmMLP::transferStreamedMatrixInputChunksToPython(const uint32 input_index)
{
	...
	for(uint32 chunk_index=0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		if (m_vDecoders[input_index]->isHeaderReceived())
		{
			...
			IMatrix* l_pMatrix = ( (TStreamedMatrixDecoder <CBoxAlgorithmMLP> *) m_vDecoders[input_index] )->getOutputMatrix();
...
OpenViBE::boolean CBoxAlgorithmMLP::transferStreamedMatrixOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IMatrix* l_pMatrix = ( (TStreamedMatrixEncoder <CBoxAlgorithmMLP> *) m_vEncoders[output_index] )->getInputMatrix();
...
OpenViBE::boolean CBoxAlgorithmMLP::transferSignalInputChunksToPython(const uint32 input_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
	...
	for(uint32 chunk_index=0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		{
			uint32 l_ui32DimensionCount, l_ui32DimensionSize;
			IMatrix* l_pMatrix = ( (TSignalDecoder <CBoxAlgorithmMLP> *) m_vDecoders[input_index] )->getOutputMatrix();
			l_ui32DimensionCount = l_pMatrix->getDimensionCount();
			...
			if (PyTuple_SetItem(l_pArg, 4, PyInt_FromLong((long) ( (TSignalDecoder <CBoxAlgorithmMLP> *) m_vDecoders[input_index] )->getOutputSamplingRate())) != 0)
			{
...
OpenViBE::boolean CBoxAlgorithmMLP::transferSignalOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IMatrix* l_pMatrix = ( (TSignalEncoder <CBoxAlgorithmMLP> *) m_vEncoders[output_index] )->getInputMatrix();
	...
	for (uint32 chunk_index=0; chunk_index < l_ui32OutputLen; chunk_index++)
	{
		...
		if (PyObject_IsInstance(l_pOVChunk, CBoxAlgorithmPython::m_pOVSignalHeader) == 1)
		{
			...
			OpenViBE::Kernel::TParameterHandler < OpenViBE::uint64 >& l_pSamplingRate = ( (TSignalEncoder <CBoxAlgorithmMLP> *) m_vEncoders[output_index] )->getInputSamplingRate();
...
OpenViBE::boolean CBoxAlgorithmMLP::transferStimulationInputChunksToPython(const uint32 input_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();
	//Borrowed reference
	PyObject *l_pBuffer = PyList_GetItem(m_pBoxInput, (Py_ssize_t) input_index);
	...
	for(uint32 chunk_index = 0; chunk_index < l_rDynamicBoxContext.getInputChunkCount(input_index); chunk_index++)
	{
		...
		if (m_vDecoders[input_index]->isBufferReceived())
		{
			IStimulationSet* l_pStimulationSet = ( (TStimulationDecoder <CBoxAlgorithmMLP> *) m_vDecoders[input_index] )->getOutputStimulationSet();
...
OpenViBE::boolean CBoxAlgorithmMLP::transferStimulationOutputChunksFromPython(const uint32 output_index)
{
	IBoxIO& l_rDynamicBoxContext=this->getDynamicBoxContext();

	IStimulationSet* l_pStimulationSet = ( (TStimulationEncoder <CBoxAlgorithmMLP> *) m_vEncoders[output_index] )->getInputStimulationSet();
...
OpenViBE::boolean CBoxAlgorithmMLP::process(void)
{	
    const IBox& l_rStaticBoxContext = this->getStaticBoxContext();
	OpenViBE::CIdentifier l_oTypeIdentifier;
